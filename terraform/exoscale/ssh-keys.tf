...
locals {
  ...
  ssh_keys                  = local.create_agent_ssh_key ? [for keys in exoscale_ssh_key.agent_generated : keys.id] : [for keys in exoscale_ssh_key.agent_passed : keys.id]
...
# SSH key - Generated
resource "exoscale_ssh_key" "agent_generated" {
  count = local.create_agent_ssh_key ? 1 : 0

  name       = "${lower(replace(var.agent_name, " ", "-"))}-${substr(replace(element(split(" ", join(" ", local.public_keys)), 1), "/\\W/", ""), -10, -1)}"
