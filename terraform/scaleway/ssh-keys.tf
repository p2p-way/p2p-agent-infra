...
locals {
  ...
  ssh_keys                  = local.create_agent_ssh_key ? [for keys in scaleway_iam_ssh_key.agent_generated : keys.id] : [for keys in scaleway_iam_ssh_key.agent_passed : keys.id]
...
# SSH key - User passed
resource "scaleway_iam_ssh_key" "agent_passed" {
...
# SSH key - Generated
resource "scaleway_iam_ssh_key" "agent_generated" {
  count = local.create_agent_ssh_key ? 1 : 0
