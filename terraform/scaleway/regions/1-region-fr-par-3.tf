...
module "fr-par-3" {
  ...
  instance_type            = "PRO2-XXS" # var.instance_type
  os_name                  = var.os_name
...
# Agent instances
output "agent_instances_fr-par-3" {
  value = length(module.fr-par-3.agent_instances) > 0 ? join("\n", flatten([for instance in module.fr-par-3 : instance])) : null
}
