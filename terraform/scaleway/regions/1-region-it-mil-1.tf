...
module "it-mil-1" {
  ...
  instance_type            = "BASIC3-X2C-4G" # var.instance_type
  os_name                  = var.os_name
...
# Agent instances
output "agent_instances_it-mil-1" {
  value = length(module.it-mil-1.agent_instances) > 0 ? join("\n", flatten([for instance in module.it-mil-1 : instance])) : null
}
