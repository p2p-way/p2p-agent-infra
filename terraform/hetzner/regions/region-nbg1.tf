...
module "nbg1" {
  ...
  instance_type            = lookup(var.instance_type, "nbg1", var.instance_type["default"])
  os_name                  = var.os_name
...
output "agent_instances_nbg1" {
  value = length(module.nbg1.agent_instances) > 0 ? join("\n", flatten([for instance in module.nbg1 : instance])) : null
}
