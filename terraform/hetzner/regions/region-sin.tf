...
module "sin" {
  ...
  instance_type            = lookup(var.instance_type, "sin", var.instance_type["default"])
  os_name                  = var.os_name
...
output "agent_instances_sin" {
  value = length(module.sin.agent_instances) > 0 ? join("\n", flatten([for instance in module.sin : instance])) : null
}
