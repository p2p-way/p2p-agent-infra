...
module "hnl" {
  ...
  instance_type            = "vhp-1c-1gb" # var.instance_type
  os_name                  = var.os_name
...
# Agent instances
output "agent_instances_hnl" {
  value = length(module.hnl.agent_instances) > 0 ? join("\n", flatten([for instance in module.hnl : instance])) : null
}
