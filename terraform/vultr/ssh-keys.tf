...
locals {
  ...
  ssh_keys                  = local.create_agent_ssh_key ? [for keys in vultr_ssh_key.agent_generated : keys.id] : [for keys in vultr_ssh_key.agent_passed : keys.id]
...
# SSH key - User passed
resource "vultr_ssh_key" "agent_passed" {
