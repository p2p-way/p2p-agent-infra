...
output "instance_private_key" {
  ...
  value       = try(tls_private_key.agent[0].private_key_openssh, "")
  sensitive   = true
}
...
output "instance_public_key" {
  ...
  value       = try(chomp(tls_private_key.agent[0].public_key_openssh), "")
}

# SSH private key - Repository
output "repository_private_key" {
  description = "Repository SSH private key"
  value       = try(tls_private_key.repository[0].private_key_openssh, "")
  sensitive   = true
}
...
output "repository_public_key" {
  ...
  value       = try(chomp(tls_private_key.repository[0].public_key_openssh), "")
}
