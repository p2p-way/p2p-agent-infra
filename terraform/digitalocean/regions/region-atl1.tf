...
module "atl1" {
  ...
  instance_type            = "s-1vcpu-1gb-amd" # var.instance_type
  os_name                  = var.os_name
...
# Agent instances
output "agent_instances_atl1" {
  value = length(module.atl1.agent_instances) > 0 ? join("\n", flatten([for instance in module.atl1 : instance])) : null
}
