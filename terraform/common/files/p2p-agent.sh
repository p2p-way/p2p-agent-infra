...
ENABLE_${command}="${value}"
%{ endfor ~}

# Commands defaults
%{ for command_default, value in commands_defaults ~}
${command_default}=%{ if length(regexall("\"", "${value}" )) > 0 }'${value}'%{ else }%{ if can(tonumber(value)) }${value}%{ else }"${value}"%{ endif }%{ endif }
%{ endfor ~}
...
CC_COMMANDS="${cc_commands}"
CC_COMMANDS_PREFIX="${cc_commands_prefix}"
